...
module.exports = function commitVerify (path) {
  ...
  try {
    ...
    if (show.match('Initial commit')) {
      addToList(i18n.t("Can't find committed changes."), false)
      helper.challengeIncomplete()
    } else if (show.match('nothing to commit')) {
      addToList(i18n.t('Changes have been committed!'), true)
      markChallengeCompleted(currentChallenge)
      userData.updateData(currentChallenge)
    } else {
      addToList(i18n.t('Seems there are still changes to commit.'), false)
      helper.challengeIncomplete()
    }
  } catch (err) {
