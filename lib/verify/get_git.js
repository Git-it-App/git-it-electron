...
module.exports = function verifyGetGitChallenge () {
  ...
  try {
    ...
    if (gitOutput.match('git version')) {
      counter++
      addToList(i18n.t('Found Git installed!'), true)
    } else {
      addToList(i18n.t('Did not find Git installed.'), false)
  ...
  try {
    ...
    if (name !== '') {
      counter++
      addToList(i18n.t('Name Added!'), true)
    } else {
      addToList(i18n.t('No name found.'), false)
  ...
  try {
    ...
    if (email !== '') {
      counter++
      addToList(i18n.t('Email Added!'), true)
    } else {
      addToList(i18n.t('No email found.'), false)
      helper.challengeIncomplete()
    }
  } catch (err) {
    console.log(err)
