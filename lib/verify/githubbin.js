...
module.exports = async function verifyGitHubbinChallenge () {
  ...
  try {
    ...
    if (username === '') {
      addToList(i18n.t('No username found.'), false)
      return helper.challengeIncomplete()
    } else {
      counter++
      addToList(i18n.t('Username added to Git config!'), true)
    }
  } catch (err) {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      addToList(i18n.t('GitHub account matching stored Git-username was not found.'), false)
      return helper.challengeIncomplete()
    }
  } catch (err) {
  ...
  if (username === githubUsername) {
    counter++
    addToList(i18n.t('Username identical on GitHub and Git config!'), true)
  } else {
    addToList(i18n.t('GitHub & Git config usernames do not match.'), false)
