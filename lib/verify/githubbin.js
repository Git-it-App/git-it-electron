...
module.exports = async function verifyGitHubbinChallenge () {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      addToList("GitHub account matching stored Git-username wasn't found.", false)
      return helper.challengeIncomplete()
    }
  } catch (err) {
  ...
  if (username === githubUsername) {
    counter++
    addToList('Username identical on GitHub and Git config!', true)
  } else {
    addToList('GitHub & Git config usernames do not match.', false)
