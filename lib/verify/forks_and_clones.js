...
module.exports = function verifyForksAndClonesChallenge (path) {
  ...
  try {
    ...
    if (remotes.length !== 4) {
      addToList(i18n.t('verify~Did not find 2 remotes set up.'), false)
      return helper.challengeIncomplete()
    }
  } catch (err) {
    addToList('Error: ' + err.message, false)
  ...
  remotes.forEach(remote => {
    if (remote.match('origin')) {
      if (remote.match('github.com[:/]' + username + '/')) {
        addToList(i18n.t('verify~Origin points to your fork!'), true)
      } else {
        incomplete = true
        addToList(i18n.t('verify~No Origin remote found pointing to {/userrepo/}.', { userrepo: username + '/patchwork' }), false)
      }
    }

    if (remote.match('upstream')) {
      if (remote.match('github.com[:/]jlord/')) {
        addToList(i18n.t('verify~Upstream remote set up!'), true)
      } else {
        incomplete = true
        addToList(i18n.t('verify~No Upstream remote found pointing to {/patchworkrepo/}.', { patchworkrepo: 'jlord/patchwork' }), false)
      }
    }
  })
