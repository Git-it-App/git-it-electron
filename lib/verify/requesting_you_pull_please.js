...
module.exports = async function verifyPRChallenge () {
  ...
  try {
    ...
    if (response.status === 200) {
      ...
      if (pr) {
        addToList(i18n.t('Found your pull request!'), true)
        markChallengeCompleted(currentChallenge)
        userData.updateData(currentChallenge)
      } else {
        // TODO give user url to their repo also
        addToList(i18n.t('No merged pull request found for username {{username}}. If you created a pull request on github, return there to see if reporobot left some comments.',
          { username }), false)
