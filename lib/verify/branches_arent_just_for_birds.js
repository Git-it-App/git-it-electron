...
module.exports = function verifyBranchesChallenge (repopath) {
  ...
  try {
    ...
    if (currentBranch.match(expectedBranch)) {
      counter++
      addToList(i18n.t('Found branch as expected!'), true)
    } else {
      addToList(i18n.t('Branch name expected: {{expectedBranch}}', { expectedBranch }), false)
  ...
  try {
    ...
    if (allFiles.match('add-' + username)) {
      counter++
      addToList(i18n.t('File in contributors folder!'), true)
    } else {
      addToList(i18n.t('File not found in contributors folder.'), false)
      helper.challengeIncomplete()
    }
  } catch (err) {
  ...
  try {
    ...
    if (log.match('update by push')) {
      counter++
      addToList(i18n.t('Changes have been pushed!'), true)
    } else {
      addToList(i18n.t('Changes not pushed.'), false)
      helper.challengeIncomplete()
    }
  } catch (err) {
