...
module.exports = function verifyMergeTadaChallenge (path) {
  ...
  try {
    ...
    if (ref.match('merge add-' + username)) {
      counter++
      addToList(i18n.t('Your branch has been merged!'), true)
    } else {
      addToList(i18n.t('No merge of your branch in history.'), false)
    }
  } catch (err) {
    addToList('Error: ' + err.message, false)
    return helper.challengeIncomplete()
  ...
  try {
    ...
    if (branches.match('add-' + username)) {
      addToList(i18n.t('Uh oh, your branch is still there.'), false)
    } else {
      counter++
      addToList(i18n.t('Branch deleted!'), true)
    }
  } catch (err) {
    addToList('Error: ' + err.message, false)
    return helper.challengeIncomplete()
