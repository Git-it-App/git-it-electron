...
module.exports = function verifyRemoteControlChallenge (path) {
  ...
  try {
    ...
    if (remotes.match('origin')) {
      addToList(i18n.t('Found remote set up.'), true)
    } else {
      addToList(i18n.t('Did not find remote origin.'), false)
      return helper.challengeIncomplete()
    }
  } catch (err) {
  ...
  try {
    ...
    if (ref.match('update by push')) {
      addToList(i18n.t('You pushed changes!'), true)
      markChallengeCompleted(currentChallenge)
      userData.updateData(currentChallenge)
    } else {
      addToList(i18n.t('No evidence of push.'), false)
      helper.challengeIncomplete()
    }
  } catch (err) {
    // TODO provide nicer error-message and only log the error?
    console.log(err.message)
    addToList(i18n.t('No pushed changes found.'), false)
    return helper.challengeIncomplete()
  }
}
