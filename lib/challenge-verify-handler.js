...
async function printOutVerifyList (list) {
  ...
  list.forEach(listItem => {
    ...
    li.appendChild(document.createTextNode(
      ipc.sendSync('i18n.t', listItem.message, listItem.data)
    ))
    if (listItem.pass) {
...
async function showSelectedDirDiv (show) {
  if (show) {
    ...
    selectDirButton.innerText = ipc.sendSync('i18n.t', 'verify~Change Directory')
    ...
    selectDirButton.innerText = ipc.sendSync('i18n.t', 'verify~Select directory')
