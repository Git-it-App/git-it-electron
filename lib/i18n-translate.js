...
async function insertDataTranslations () {
  ...
  i18nElements.forEach(element => {
    ...
    if (type === 'html' || type === 'standard_html') {
      element.innerHTML = ipc.sendSync('i18n.t', data, options)
    } else {
      element.innerText = ipc.sendSync('i18n.t', data, options)
    }
  })
}
...
async function insertTranslationStyles () {
  ...
  if (document.styleSheets[sheetIndex].ownerNode.id === 'challenge-translation-style') {
    document.styleSheets[sheetIndex].insertRule('.box--goal::before {content: "' + ipc.sendSync('i18n.t', 'Goal') + '";}')
    document.styleSheets[sheetIndex].insertRule('.box--fail::before {content: "' + ipc.sendSync('i18n.t', "Didn't Pass?") + '";}')
    document.styleSheets[sheetIndex].insertRule('.box--tip::before {content: "' + ipc.sendSync('i18n.t', 'Tip') + '";}')
    document.styleSheets[sheetIndex].insertRule('.box--step::before {content: "' + ipc.sendSync('i18n.t', 'Step') + '";}')
  }
}
