...
function buildHeader (filename) {
  const challengeTitle = makeTitleName(filename)

  const content = {
    challengeTitle,
    localeMenu: new Handlebars.SafeString(getLocaleMenu())
  }
  return partialTemplates.chalHeader(content)
}
...
function getPartialTemplates () {
  return {
    chalHeader: Handlebars.compile(fs.readFileSync(path.join(partialsFolder, 'chal-header.html')).toString().trim()),
    chalFooter: Handlebars.compile(fs.readFileSync(path.join(partialsFolder, 'chal-footer.html')).toString().trim()),
    chalSidebar: Handlebars.compile(fs.readFileSync(path.join(partialsFolder, 'chal-sidebar.html')).toString().trim())
  }
}
...
function getPartialButtons () {
  return {
    verify_button: fs.readFileSync(path.join(partialsFolder, 'verify-button.html')).toString().trim(),
    verify_directory_button: fs.readFileSync(path.join(partialsFolder, 'verify-directory-button.html')).toString().trim()
  }
}
